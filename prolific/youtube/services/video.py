...
class VideoAssemblyService:
    ...
    async def create_ken_burns_clip(
        ...
        zoom_filters = {
            "zoom_in": f"zoompan=z='min(zoom+0.0001,1.3)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d={total_frames}:s=1920x1080:fps={fps}",
            "zoom_out": f"zoompan=z='if(eq(on,0),1.3,max(zoom-0.0001,1.0))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d={total_frames}:s=1920x1080:fps={fps}",
            "pan_left": f"zoompan=z='1.1':x='iw*0.1+on/(on+1)*iw*0.1':y='ih/2-(ih/zoom/2)':d={total_frames}:s=1920x1080:fps={fps}",
            "pan_right": f"zoompan=z='1.1':x='iw*0.3-on/(on+1)*iw*0.1':y='ih/2-(ih/zoom/2)':d={total_frames}:s=1920x1080:fps={fps}",
        }
